...
After an *act* have acted the claims are checked to be true by implementations of the `IAssertor` interface (reading calims from an instance of `ISystemClaims` and comparing claim values with the state read from the system itself).
